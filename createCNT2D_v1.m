...
%%In this code, carbon nanotube can only rotation along Z direction.
%%Developed in 2017
tic;
clear;clc;
close all;
...
%ssgraphatom_ind_0=47; %%test certain structure
slct_atom_index(1)=ssgraphatom_ind_0;
%slct_atom_index(1,m+n+1)=ssgraphatom_ind_0;  %%test certain structure
...
sitaarray=zeros(1,length_ssgraphcrd);
...
for exh=1:length_ssgraphcrd
    ...
    [subf,X,Y,sita,h]=JoiningCNT(ssgraph,cc_temp,a,n,m,z);
    Xarray(exh)=X;
    Yarray(exh)=Y;
    sitaarray(exh)=sita;
    harray(exh)=h;
...
for exh=1:length(subfarray_idx)
    ...
    sita=sitaarray(subfarray_idx(exh));
    ...
    T2=[cos(sita),-sin(sita);sin(sita),cos(sita)];
    cc_new=T2*cctrans1;
...
c_temp1=Trans_T*c;
T3=[cos(sita),-sin(sita),0;sin(sita),cos(sita),0;0,0,1];
...
ccnew=[T2*cctrans2;cnew(3,lift_atoms_cnt)];%%CNT bottom atom coordinate
...
if (m~=n) && (m~=0)
    ...
    for i_lift_atoms_dists=1:sizecc
        ...
        if lift_atoms_dists(i_lift_atoms_dists) < D_cutoff
            ...
            nn_lift_atoms_dists=find(neighbor_list(slct_G(i_lift_atoms_dists),:)==1);
...
z_scgraphnew_up=max(cnew(3,:))+min(cnew(3,:))-min(scgraphnew(3,:));
scgraphnew_up=z_scgraphnew_up*ones(1,size(scgraphnew,2));
cntgrph=[scgraphnew(1,:),cnew(1,:);scgraphnew(2,:),cnew(2,:);scgraphnew(3,:),cnew(3,:)];
cntgrph_swch=[scgraphnew(1,:),cnew(1,:),scgraphnew(1,:);scgraphnew(2,:),cnew(2,:),scgraphnew(2,:);scgraphnew(3,:),cnew(3,:),scgraphnew_up];
if sandwich==1
    cntgrph=cntgrph_swch;
end
